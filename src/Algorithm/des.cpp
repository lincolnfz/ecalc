...
void des_xor(uint8 data[8], const uint8 vi[8]){
	int i = 0;
	for(i = 0; i < 8;i++){
		data[i] ^= vi[i];
	}
}
int des_padding(uint8 data[8]){
	int nSize = 0;
	int i = 7;
	int nCount = 0;
	unsigned char padding_byte = data[7];
	for(i = 7; i >= 0;i--){
		if(data[i] == padding_byte)
			nCount++;
	}
	if(padding_byte == nCount){
		for(i = 7; i > (7 - nCount);i--){
			if(data[i] == padding_byte)
					data[i] = 0;
			}
		nSize = nCount;
...
int des_cbc_encrypt( des_context *ctx, const uint8 *input,int len, uint8 *output, const uint8 vi[8])
{
	uint8 data[8] = {0};
	int max = get_max_buffer_length(len);
	...
	for(; i < max; i++){
		...
		if((i % 8) == 7){
			if(j == 0)
				...
				des_xor(data,output +  (j - 1) * 8);
			des_crypt( ctx->esk, data, output +  j * 8);
			j++;
		}
	}
...
int des_cbc_decrypt( des_context *ctx, const uint8 *input,int len, uint8 *output, const uint8 vi[8])
{
	uint8 data[8] = {0};
	int i = len - 1;
	int j = 0;
	...
	for(; i >= 0; i--){
		data[i % 8] = input[i];
		if((i % 8) == 0){
			des_crypt( ctx->dsk, data, output + ((len / 8) - j - 1) * 8);
			int n = ((len / 8) - j - 1) * 8;
			if(i == 0)
				des_xor(output + ((len / 8) - j - 1) * 8,vi);
			else
				des_xor(output + ((len / 8) - j - 1) * 8,input + ((len / 8) - j - 2) * 8);
			if(j == 0)
				padding = des_padding(output + ((len / 8) - j - 1) * 8);
			j++;
		}
	}
	return len - padding;
